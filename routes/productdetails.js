...
router.post('/submit_product_details', upload.any(), function (req, res, next) {
  try {
    ...
    pool.query(
      'insert into productdetails (categoryid,subcategoryid,brandid,productid,productsubname,concernid,description,weight,weighttype,type,packaging,qty,price,offerprice,offertype,picture) values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)',
      [
        req.body.categoryid,
    ...
    pool.query(
      ...
      function (error, result) {
        if (error) {
          ...
          res
            .status(200)
            .json({ status: true, message: 'Pictures Edited Successfully!' });
        }
      },
    );
  } catch (e) {
