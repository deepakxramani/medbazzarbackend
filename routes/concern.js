...
router.post(
  ...
  function (req, res, next) {
    ...
    try {
      pool.query(
        'insert into concerns (concernname,picture) values(?,?)',
        ...
        function (error, result) {
          if (error) {
            ...
            res.status(200).json({
              status: true,
              message: 'Concern Details Submitted Successfully...',
              data: result,
            });
          }
        },
...
router.post('/edit_concern_data', function (req, res, next) {
  ...
  try {
    pool.query(
      'update concerns set concernname=? where concernid=?',
      [req.body.concernname, req.body.concernid],
      function (error, result) {
        if (error) {
          res.status(200).json({
            status: false,
          ...
          res
            .status(200)
            .json({ status: true, message: 'Concern Updated Successfully...' });
        }
      },
    );
  } catch (e) {
...
router.post(
  ...
  function (req, res, next) {
    ...
    try {
      pool.query(
        'update concerns set picture=? where concernid=?',
        [req.file.filename, req.body.concernid],
        function (error, result) {
          if (error) {
            res.status(200).json({
              status: false,
...
router.post('/delete_concern_data', function (req, res, next) {
  ...
  try {
    pool.query(
      'delete from concerns where concernid=?',
      [req.body.concernid],
      function (error, result) {
        if (error) {
          console.log(error);
